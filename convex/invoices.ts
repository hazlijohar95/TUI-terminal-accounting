  ...
  handler: async (ctx, args) => {
    const invoice = await ctx.db
      ...
      .withIndex("by_number", (q) =>
        q.eq("orgId", args.orgId).eq("number", args.number)
      )
      .first();
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.taxRate !== undefined) {
      ...
      updates.total = invoice.subtotal + (updates.taxAmount as number);
      updates.balanceDue = (updates.total as number) - invoice.amountPaid;
    }
